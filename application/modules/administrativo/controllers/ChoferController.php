...
class Administrativo_ChoferController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $parametrosNamespace->jsGrip = '/js/grillasmodulos/administrativo/gridChofer.js';
        $parametrosNamespace->Application_Model_DbTable = "Application_Model_DbTable_Chofer";
        $parametrosNamespace->busqueda = "nombre_chofer";
...
//        die($select);
	...
	private function obtenerPaginas($result,$cantidadFilas,$page){
		...
		foreach ( $this->_paginator as $item ) {       
			$arrayDatos ['cell'] = array(
                            ...
                            trim(utf8_encode($item["fecha_nacimiento"])),
                            $item["telefono_chofer"],
                            $item["cod_proveedor"]);
    ...
    public function eliminarAction(){
		...
		try{
                    ...
                    $db->beginTransaction();
                    $servCon = new Application_Model_DataService($parametrosNamespace->Application_Model_DbTable);
                    $servCon->deleteRowById(array("cod_chofer"=>$id));
                    $db->commit();
	...
	public function guardarAction(){
            ...
            $rowData = json_decode($json_rowData);
            $applicationModel = new Application_Model_Chofer();
	...
	public function modificarAction(){
            ...
            $json_rowData = $this->getRequest ()->getParam ( "parametros" );
            $rowData = json_decode($json_rowData);
            $rowClass = new Application_Model_Chofer();
		if($rowData->idRegistro != null){
				$rowClass->setCod_chofer($rowData->idRegistro);
			}
        ...
        public function almacenardatos($rowClass,$rowData){
     	try{
        	...
        	$db->beginTransaction();
                ...
                $rowClass->setNombre_chofer(trim(utf8_decode($rowData->nombre_chofer)));
                $rowClass->setFecha_nacimiento(trim(utf8_decode($rowData->fecha_nacimiento)));
                $rowClass->setTelefono_chofer(trim(utf8_decode($rowData->telefono_chofer)));
