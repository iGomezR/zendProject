table = "/administrativo/Ordendeentrada/";
mensajeWarning = new Array(
        "Debe seleccionar un proveedor, por favor",
        "Debe seleccionar un camion, por favor",
        "Debe seleccionar un chofer,por favor",
        "Debe seleccionar un analista, por favor",
        'Debe ingresar peso de entrada,por favor');
idCamposGrilla = new Array("id-registro","nro_orden_de_entrada","selectProveedor",
"selectCamion","selectChofer","selectUsuario",'peso_total');

$().ready(function() {
        cargarProveedor();
        cargarUsuario();        
        $("#"+idCamposGrilla[2]).datepicker();
        $("#"+idCamposGrilla[2]).datepicker("option", "dateFormat", "yy-mm-dd");        
...
function obtenerJsonFormulario() {
	...
	if($('#'+idCamposGrilla[2]).attr("value") == -1 || $('#'+idCamposGrilla[2]).attr("value").length == 0){
            mostarVentana("warning-registro",mensajeWarning[0]);
            $('#'+idCamposGrilla[2]).focus();
		...
		mostarVentana("warning-registro",mensajeWarning[1]);
                $('#'+idCamposGrilla[3]).focus();
	} else if($('#'+idCamposGrilla[4]).attr("value") == -1 || $('#'+idCamposGrilla[4]).attr("value").length == 0){
		mostarVentana("warning-registro",mensajeWarning[2]);
                $('#'+idCamposGrilla[4]).focus();
		...
		mostarVentana("warning-registro",mensajeWarning[3]);
                $('#'+idCamposGrilla[5]).focus();
	} else if($('#'+idCamposGrilla[6]).attr("value") == '' || $('#'+idCamposGrilla[6]).attr("value").length == 0){
		mostarVentana("warning-registro",mensajeWarning[4]);
                $('#'+idCamposGrilla[6]).focus();
	}
        else {           
            jsonObject.idRegistro = $('#'+idCamposGrilla[0]).attr("value");
            jsonObject.cod_proveedor = $("#"+idCamposGrilla[2]+" option:selected").val();
            jsonObject.cod_camion = $("#"+idCamposGrilla[3]+" option:selected").val();
            jsonObject.cod_chofer = $("#"+idCamposGrilla[4]+" option:selected").val();
            jsonObject.cod_usuario_analista = $("#"+idCamposGrilla[5]+" option:selected").val();
            jsonObject.nro_orden_de_entrada = $('#'+idCamposGrilla[1]).attr("value");
            jsonObject.peso_total = $('#'+idCamposGrilla[6]).attr("value");
            return jsonObject;
	}
    return null;
}
...
function validarNumerosCampo(){
	 $("#"+idCamposGrilla[6]).keydown(function(event) {
...
function editarRegistro(parametros){
	...
	$("#"+idCamposGrilla[0]).attr("value",parametros.idRegistro);
        $("#"+idCamposGrilla[1]).attr("value",parametros.nro_orden_de_entrada);
	$("#"+idCamposGrilla[2]+" option[value="+parametros.cod_proveedor+"]").attr("selected",true);
        cargarCamion();
        cargarChofer();         
	$("#"+idCamposGrilla[3]+" option[value="+parametros.cod_camion+"]").attr("selected",true);
	$("#"+idCamposGrilla[4]+" option[value="+parametros.cod_chofer+"]").attr("selected",true);
        $("#"+idCamposGrilla[5]+" option[value="+parametros.cod_usuario_analista+"]").attr("selected",true); 
        $("#"+idCamposGrilla[6]).attr("value",parametros.peso_total);
...
function cargarProveedor(){
	$.ajax({
        ...
        async : false,
        success: function(respuesta){
        	$("#selectProveedor").html(respuesta);
        },
        error: function(event, request, settings){
         //   $.unblockUI();
            alert("Ha ocurrido un error");
...
function cargarCamion(){
        var jsonObject = new Object();
        jsonObject.cod_proveedor = $('#selectProveedor').val();
        var dataString = JSON.stringify(jsonObject);
	$.ajax({
        url: table+'camiondata',
        type: 'post',
        data: {"parametros":dataString},
        dataType: 'html',
        async : false,
        success: function(respuesta){
        	$("#selectCamion").html(respuesta);
        },
        error: function(event, request, settings){
         //   $.unblockUI();
            alert("Ha ocurrido un error");
...
function cargarChofer(){
        ...
        var dataString = JSON.stringify(jsonObject);    
	$.ajax({
        url: table+'choferdata',
        type: 'post',
        data: {"parametros":dataString},
        dataType: 'html',
        async : false,
        success: function(respuesta){
        	$("#selectChofer").html(respuesta);
        },
        error: function(event, request, settings){
         //   $.unblockUI();
            alert("Ha ocurrido un error");
...
function cargarLoteDocumento(){
	$.ajax({
        url: table+'documentolotedata',
        type: 'post',
        dataType: 'html',
        async : false,
        success: function(respuesta){
        	$("#nro_orden_de_entrada").attr("value",respuesta);
        },
        error: function(event, request, settings){
         //   $.unblockUI();
            alert("Ha ocurrido un error");
