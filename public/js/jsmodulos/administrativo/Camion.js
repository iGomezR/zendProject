...
table = "/administrativo/camion/";
mensajeWarning = new Array(
        "Debe seleccionar un proveedor, por favor",
        "Debe ingresar la marca del camion,por favor",
        "Debe ingresar el modelo del camion,por favor",
        "Debe ingresar el anho del camion,por favor",
        "Debe ingresar el peso neto del camion,por favor",
        "Debe ingresar la chapa del camion, por favor");
idCamposGrilla = new Array("id-registro","selectProveedor","marca_camion","modelo_camion","anho_camion","peso_neto_camion","chapa_camion");
...
function obtenerJsonFormulario() {
	...
	if($('#'+idCamposGrilla[1]).attr("value") == -1 || $('#'+idCamposGrilla[1]).attr("value").length == 0){
            mostarVentana("warning-registro",mensajeWarning[0]);
            $('#'+idCamposGrilla[1]).focus();
		...
		jsonObject.idRegistro = $('#'+idCamposGrilla[0]).attr("value");
                jsonObject.cod_proveedor = $("#"+idCamposGrilla[1]+" option:selected").val();
                jsonObject.marca_camion = $('#'+idCamposGrilla[2]).attr("value");
                jsonObject.modelo_camion = $('#'+idCamposGrilla[3]).attr("value");
                jsonObject.anho_camion = $('#'+idCamposGrilla[4]).attr("value");
                jsonObject.peso_neto_camion = $('#'+idCamposGrilla[5]).attr("value");
                jsonObject.chapa_camion = $('#'+idCamposGrilla[6]).attr("value");
		return jsonObject;
	}
    return null;
}
...
function editarRegistro(parametros){
	...
	$("#"+idCamposGrilla[1]+" option[value="+parametros.cod_proveedor+"]").attr("selected",true);
	$("#"+idCamposGrilla[2]).attr("value",parametros.marca_camion);
	$("#"+idCamposGrilla[3]).attr("value",parametros.modelo_camion);
        $("#"+idCamposGrilla[4]).attr("value",parametros.anho_camion);  
        $("#"+idCamposGrilla[5]).attr("value",parametros.peso_neto_camion);
        $("#"+idCamposGrilla[6]).attr("value",parametros.chapa_camion);
