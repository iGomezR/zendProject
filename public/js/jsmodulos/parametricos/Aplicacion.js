table = "/parametricos/Aplicacion/";
mensajeWarning = new Array("","Debe ingresar una descripcion!!","Debe seleccionar un modulo");
idCamposBusqueda = new Array("descripcionBusqueda");
idCamposGrilla = new Array("id-registro","descrip-modal","modulo-modal","activo-modal");
$().ready(function() {
        cargarModulo();
...
function obtenerJsonFormulario() {
	...
	if($('#'+idCamposGrilla[1]).attr("value") == null || $('#'+idCamposGrilla[1]).attr("value").length == 0){
		...
		mostarVentana("warning-registro",mensajeWarning[2]);
                ...
                if($('#'+idCamposGrilla[3]).attr("checked")) $estado = 'S'; else $estado = 'N';
		jsonObject.idRegistro = $('#'+idCamposGrilla[0]).attr("value");
                jsonObject.desc_aplicacion = $('#'+idCamposGrilla[1]).attr("value");
                jsonObject.cod_modulo = $('#'+idCamposGrilla[2]).attr("value");
...
function editarRegistro(parametros){
	...
	$("#"+idCamposGrilla[1]).attr("value",parametros.desc_aplicacion);
	$("#"+idCamposGrilla[2]).attr("value",parametros.cod_modulo);
        if(parametros.activo == 'S')
            $('input[name='+idCamposGrilla[3]+']').attr('checked', true);            
        else
            $('input[name='+idCamposGrilla[3]+']').attr('checked', false);
...
function cargarModulo(){
	$.ajax({
        url: table+'modulodata',
        type: 'post',
        dataType: 'html',
        async : false,
        success: function(respuesta){
        	$("#modulo-modal").html(respuesta);
        },
        error: function(event, request, settings){
         //   $.unblockUI();
            alert("Ha ocurrido un error");
