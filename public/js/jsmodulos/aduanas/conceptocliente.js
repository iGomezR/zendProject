...
function enviarParametrosRegistro(data){
	...
	var urlenvio = '';
	if(data.cod_concepto_cta_cliente != null && data.cod_concepto_cta_cliente.length != 0){
...
function obtenerJsonFormulario() {
    ...
    if ($('#desc_concepto_cta_cliente').attr("value") == null || $('#desc_concepto_cta_cliente').attr("value").length == 0) {
        mostarVentana("warning-registro", "Complete la descripcion");
    }
    else {
        jsonObject.cod_concepto_cta_cliente = $('#cod_concepto_cta_cliente-modal').attr("value");
        jsonObject.desc_concepto_cta_cliente = $('#desc_concepto_cta_cliente').attr("value");
...
function validarNumerosCampo(){
	 $("#desc_concepto_cta_cliente").keydown(function(event) {
...
function editarRegistro(parametros){
	...
	$("#cod_concepto_cta_cliente-modal").attr("value",parametros.cod_concepto_cta_cliente);
	$("#desc_concepto_cta_cliente").attr("value",parametros.desc_concepto_cta_cliente);          
...
function limpiarFormulario(){
	...
	$("#success-block").hide();
        $("#cod_concepto_cta_cliente-modal").attr("value",null);
	$("#desc_concepto_cta_cliente").attr("value",null);
...
function obtenerJsonBuscar(){
	...
	if($('#descripcionBusqueda').attr("value") != null && $('#descripcionBusqueda').attr("value").length != 0){
		jsonObject.desc_concepto_cta_cliente = $('#descripcionBusqueda').attr("value");
	}
