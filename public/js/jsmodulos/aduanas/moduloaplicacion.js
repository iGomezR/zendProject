table = "/aduanas/moduloaplicacion/";
...
function enviarParametrosRegistro(data){
	...
	var urlenvio = '';
	if(data.cod_modulo != null && data.cod_modulo.length != 0){
...
function obtenerJsonFormulario() {
    ...
    if ($('#desc_modulo').attr("value") == null || $('#desc_modulo').attr("value").length == 0) {
        mostarVentana("warning-registro", "Complete la descripcion");
    }
    else {
        if($('#activo').attr("checked")) $estado = 'S'; else $estado = 'N';
        jsonObject.cod_modulo = $('#cod_modulo-modal').attr("value");
        jsonObject.desc_modulo = $('#desc_modulo').attr("value");
...
function validarNumerosCampo(){
	 $("#desc_modulo").keydown(function(event) {
...
function editarRegistro(parametros){
	...
	$("#cod_modulo-modal").attr("value",parametros.cod_modulo);
	$("#desc_modulo").attr("value",parametros.desc_modulo);       
...
function limpiarFormulario(){
	...
	$("#success-block").hide();
        $("#cod_modulo").attr("value",null);
	$("#desc_modulo").attr("value",null);
...
function obtenerJsonBuscar(){
	...
	if($('#descripcionBusqueda').attr("value") != null && $('#descripcionBusqueda').attr("value").length != 0){
		jsonObject.desc_modulo = $('#descripcionBusqueda').attr("value");
	}
...
function cleanModal(){
    $("#cod_modulo-modal").attr("value",null);
    $("#desc_modulo").attr("value",null);
    $("#activo").attr("value",null);
