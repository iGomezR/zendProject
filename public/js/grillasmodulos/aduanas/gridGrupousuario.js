table = "/factura/proyectoZend/public/index.php/aduanas/grupousuario/";
//table = "/proyectoZend/public/index.php/aduanas/grupousuario/";
    ...
    jQuery("#grillaRegistro").jqGrid({
        ...
        loadComplete: function(data) {
                    ...
                    if (data.editar == false) {
                        // Si el usuario no puede Modificar se oculta la
                        // columna
                        // El Nombre de la columna esta en blanco
                        $("#grillaRegistro").jqGrid('hideCol', 'modificar');
                    }
                    if (data.borrar == false) {
                        // Si el usuario puede borrar datos se inserta
                        // el boton de borrar.
                        $("#grillaRegistro").jqGrid();
                        $('#eliminar').hide();
                    ...
                    if(data.ver == false){
                        $("#buscarregistro").hide();
                         $("#grillaRegistro").jqGrid('setGridState','hidden');
                    }
                    desbloquearPantalla();
                },
        colModel:
...
function borrar(){
	...
	id = $("#grillaRegistro").jqGrid('getCell', id, 'cod_grupo');
	if( id == false ){
...
function cargarLinkModificar(cellvalue, options, rowObject)
{
    ...
    parametros.cod_grupo = rowObject[1];
    parametros.desc_grupo = rowObject[2];
    if(rowObject[3] === "Activo"){ parametros.activo = "S";}
    if(rowObject[3] ===  "Desactivado"){  parametros.activo = "N";}
//    parametros.activo = rowObject[3];
